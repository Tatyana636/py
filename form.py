...
class Form:
        ...
        for arg in args:
            ...
            for key in self._email_.keys():
                if str.lower(arg.get_attribute(key)) in self._email_[key]:
                    self.email = arg
                    break
        print(DataToXpath({"tag": "input", "attrib": self.getAttribute(self.email)}))
    ...
    def action(self, obj, act: str, data=None):
        def do(obj, act, data):
            if act == "send_keys":
                ...
                obj.send_keys(data)
            if act == "typewrite":
                if(self.pop_up_btn_press):
                    location = obj.location 
                    pyautogui.click(x=location['x']+1990, y=989, clicks=1) 
                    pyautogui.typewrite(data)
                else:
                    location = obj.location 
                    self.driver.execute_script(f"window.scrollTo({location['x']}, {location['y']-850});")
                    pyautogui.click(x=location['x']+1990, y=989, clicks=1) 
                    pyautogui.typewrite(data)
            elif act == "click":
                obj.click()
    ...
    def Test(self):
        if self.isready:
            ...
            if self.callButton is not None:
                self.action(obj=self.callButton, act="click")
                self.pop_up_btn_press = True
            self.action(obj=self.name, act="send_keys", data=self.__nameDefault__)
            self.action(obj=self.phone, act="typewrite", data=self.__phoneDefault__)
    ...
    def checkResponse(body: str) -> bool:     
        if re.search(r'71234567890', body).group(0):
            return True
